...
import com.vuforia.samples.VideoPlayback2.app.VideoPlayback.VideoPlayerHelper.MEDIA_TYPE;
...
public class VideoPlaybackRenderer implements GLSurfaceView.Renderer, SampleAppRendererControl {
    ...
    //XXX
    private float offset = 0.0f; //regione 70.0f cellular 1000.0f
    private float pad = 1.02f; //regione cellular 1.03f;
    ...
    public VideoPlaybackRenderer(VideoPlayback activity, SampleApplicationSession session) {
        ...
        //XXX
        mSampleAppRenderer = new SampleAppRenderer(this, mActivity, Device.MODE.MODE_AR, false, 10f, 50000f); //5000f
    ...
    void initRendering() {
        ...
        videoPlaybackShaderID = SampleUtils.createProgramFromShaderSrc(VideoPlaybackShaders.VIDEO_PLAYBACK_VERTEX_SHADER, VideoPlaybackShaders.VIDEO_PLAYBACK_FRAGMENT_SHADER);
        ...
        keyframeQuadAspectRatio[VideoPlayback.BUSINESS] = (float) mTextures.get(0).mHeight / (float) mTextures.get(0).mWidth;
        keyframeQuadAspectRatio[VideoPlayback.HOPE] = (float) mTextures.get(1).mHeight / (float) mTextures.get(1).mWidth;
        ...
        //XXX
        Spanned html = Html.fromHtml("<b><a href='http://goodcode.ch/'style='text-decoration: none'>Goodcode DEMO</a></b>"); //"More information " + "<br/>   ; color:black;
        mActivity.showCard("Goodcode DEMO", html, mActivity.getBitmapFromAsset("goodWhite.png"));
    ...
    public void renderFrame(State state, float[] projectionMatrix) {
        ...
        for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++) {
            ...
            //XXX
            if (imageTarget.getName().compareTo("businesscard") == 0) //businesscard CellularlineA4
                currentTarget = VideoPlayback.HOPE;
            else
                break;
               // currentTarget = VideoPlayback.BU;

            //XXX
            VirtualButtonResult playResult = imageTargetResult.getVirtualButtonResult("play");
            if (playResult != null) {
                VirtualButton play = playResult.getVirtualButton();
                if ((System.currentTimeMillis() - startTime > 2000) && playResult.isPressed()) {
                    Log.d(LOGTAG, "playResult.isPressed() = " + playResult.isPressed());
                    mActivity.playVideo(currentTarget);
            ...
            if ((currentStatus[currentTarget] == VideoPlayerHelper.MEDIA_STATE.READY)
                ...
                //XXX
                Matrix.translateM(modelViewMatrixKeyframe, 0, 0.0f, offset, 0.0f);
                Matrix.scaleM(modelViewMatrixKeyframe, 0,
                        targetPositiveDimensions[currentTarget].getData()[0],
                        targetPositiveDimensions[currentTarget].getData()[0] * ratio,
                        targetPositiveDimensions[currentTarget].getData()[0]);

                Matrix.scaleM(modelViewMatrixKeyframe, 0, pad, pad, pad);
            ...
            // the actual contents
            {
                if (currentStatus[currentTarget] != VideoPlayerHelper.MEDIA_STATE.PAUSED) {
                    ...
                    //XXX
                    Matrix.translateM(modelViewMatrixVideo, 0, 0.0f, offset, 0.0f);
                    ...
                    Matrix.scaleM(modelViewMatrixVideo, 0, pad, pad, pad);
                    ...
                    if (imageTarget.getName().compareTo("stones") == 0)
                        ...
                        GLES20.glVertexAttribPointer(videoPlaybackTexCoordHandle, 2, GLES20.GL_FLOAT, false, 0, fillBuffer(videoQuadTextureCoordsTransformedChips));
            ...
            if ((currentStatus[currentTarget] == VideoPlayerHelper.MEDIA_STATE.READY)
                ...
                Matrix.translateM(modelViewMatrixButton,
                                0,
                                0.0f,
                                0.0f,
                                targetPositiveDimensions[currentTarget].getData()[1] / 10.98f);
                //XXX
                Matrix.translateM(modelViewMatrixButton, 0, 0.0f, offset , 0.0f); //- 0.01f
                Matrix.scaleM(modelViewMatrixButton,
                                0,
                                (targetPositiveDimensions[currentTarget].getData()[1] / 2.0f),
                                (targetPositiveDimensions[currentTarget].getData()[1] / 2.0f),
                                (targetPositiveDimensions[currentTarget].getData()[1] / 2.0f));

                Matrix.scaleM(modelViewMatrixButton, 0, pad/2, pad/2, pad/2);


                Matrix.multiplyMM(modelViewProjectionButton, 0, projectionMatrix, 0, modelViewMatrixButton, 0);
    ...
    void setVideoDimensions(int target, float videoWidth, float videoHeight, float[] textureCoordMatrix) {
        ...
        if (target == VideoPlayback.BUSINESS) {
        ...
        } else if (target == VideoPlayback.HOPE) {
            tempUVMultRes = uvMultMat4f(
